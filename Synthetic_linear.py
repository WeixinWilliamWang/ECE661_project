...
for problem in problems:
  # Skip simulation if all result files exist
  result_tags = ["LinUCB","e-greedy","LinTS","LinDiffTS","DLTS","DPSG","DMAPSG"]
  if all(os.path.exists(f"Results_linear/{problem}_{tag.lower()}.npy") for tag in result_tags):
  ...
  for super_run in range(num_super_runs_local):
    ...
    joblib.dump(prior, "Results_linear/%s_diffusion_model.pkl" % problem)
    ...
    if super_run == 0:
      ...
      plt.tight_layout()
      plt.legend()
      plt.savefig("Results_linear/%s_diffusion_prior.pdf" % problem)
      plt.close()

      np.save("Results_linear/%s_prior.npy" % problem, subS0)
      np.save("Results_linear/%s_diffusion_prior.npy" % problem, S)
      ...
      plt.tight_layout()
      plt.legend()
      plt.savefig("Results_linear/%s_prior.pdf" % problem)
      plt.close()
    ...
    if reward_model == "linear":
      algs = [
        ...
        ("LinDiffDMAP", {"prior": prior, "sigma": sigma, "K_inner": K_inner, "eta": dmap_eta}, "tab:pink", "-", "DPSG-MP")]
        ("LinDiffDMAPNew", {"prior": prior, "sigma": sigma, "K_inner": K_inner_new, "eta": dmap_eta_new}, "tab:pink", "-", "DPSG-MP-New")]
    elif reward_model == "logistic":
      algs = [
        ("LogTS", {}, "blue", "-", "TS"),
    ...
    for alg in algs:
      ...
      if super_run > 0:
        old_regret = np.load("Results_linear/%s_%s.npy" % (problem, alg[-1].lower()))
        regret = np.hstack((old_regret, regret))
      np.save("Results_linear/%s_%s.npy" % (problem, alg[-1].lower()), regret)
    print()
  ...
  for alg in algs:
    regret = np.load("Results_linear/%s_%s.npy" % (problem, alg[-1].lower()))
  ...
  plt.savefig("Results_linear/%s_%s.pdf" % (problem, alg[-1].lower()))
  plt.close()