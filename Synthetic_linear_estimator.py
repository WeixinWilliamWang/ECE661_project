...
parser.add_argument(
  ...
  type=str,
  default="all",
  help="Comma separated list of algorithms (by name or label) to run, or 'all'",
)
...
LIN_DIFF_DMAP_NEW_PARAMS = {
  "cross": {"K_inner": 2, "dmap_eta": 0.01},
  "rays": {"K_inner": 1, "dmap_eta": 0.05},
  "triangles": {"K_inner": 2, "dmap_eta": 0.1},
  "swirl": {"K_inner": 1, "dmap_eta": 0.01},
  "H": {"K_inner": 5, "dmap_eta": 0.001},
  "corners": {"K_inner": 5, "dmap_eta": 0.001},
}

MSE_TARGET_ALGS = {"LinDiffDPTS", "LinDiffDPS", "LinDiffDMAP", "LinDiffDMAPNew"}


def compute_final_theta_mse(alg_logs, envs):
  """Return per-run MSE between the last theta sample and true theta."""
  mse_vals = []
  for env_instance, alg_instance in zip(envs, alg_logs):
    theta_est = getattr(alg_instance, "theta_tilde", None)
    if theta_est is None:
      continue
    diff = np.asarray(theta_est) - np.asarray(env_instance.theta)
    mse_vals.append(np.mean(np.square(diff)))
  return np.asarray(mse_vals)


def filter_algorithms(algs, selection):
  """Filter algorithms list based on comma-separated CLI selection."""
  if selection.lower() == "all":
    return algs

  requested = {token.strip().lower() for token in selection.split(",") if token.strip()}
  if not requested:
    raise ValueError("No valid algorithm names provided in --algs.")

  filtered = [
    alg for alg in algs
    if (alg[4].lower() in requested) or (alg[0].lower() in requested)
  ]
  if not filtered:
    available = ", ".join(f"{name[0]} ({name[4]})" for name in algs)
    raise ValueError(f"--algs selection '{selection}' did not match any algorithms. "
...
for problem in problems:
  ...
  for super_run in range(num_super_runs):
    ...
    joblib.dump(prior, "Results_linear_dmap_new/%s_diffusion_model.pkl" % problem)
    ...
    if super_run == 0:
      ...
      plt.tight_layout()
      plt.legend()
      plt.savefig("Results_linear_dmap_new/%s_diffusion_prior.pdf" % problem)
      plt.close()

      np.save("Results_linear_dmap_new/%s_prior.npy" % problem, subS0)
      np.save("Results_linear_dmap_new/%s_diffusion_prior.npy" % problem, S)
      ...
      plt.tight_layout()
      plt.legend()
      plt.savefig("Results_linear_dmap_new/%s_prior.pdf" % problem)
      plt.close()
    ...
    if reward_model == "linear":
      ...
      dmap_new_cfg = LIN_DIFF_DMAP_NEW_PARAMS.get(problem, {
        "K_inner": 1,
        "dmap_eta": 0.05,
      })
      ...
      algs = [
        ...
        ("LinDiffDMAPNew", {"prior": prior, "sigma": sigma, "K_inner": dmap_new_cfg["K_inner"], "eta": dmap_new_cfg["dmap_eta"]}, "tab:pink", "-", "DPSG-MP-New")]
      algs = filter_algorithms(algs, args.algs)
    elif reward_model == "logistic":
      algs = [
        ("LogTS", {}, "blue", "-", "TS"),
    ...
    for alg in algs:
      ...
      start = time.time()
      regret, alg_logs = evaluate(alg_class, alg[1], envs, n, printout=False)

      mse_stats = None
      if alg[0] in MSE_TARGET_ALGS:
        mse_values = compute_final_theta_mse(alg_logs, envs)
        if mse_values.size > 0:
          mse_path = "Results_linear_dmap_new/%s_%s_theta_mse.npy" % (problem, alg[-1].lower())
          if super_run > 0 and os.path.exists(mse_path):
            old_mse = np.asarray(np.load(mse_path))
            mse_values = np.hstack((old_mse, mse_values))
          np.save(mse_path, mse_values)
          mse_stats = (
            float(mse_values.mean()),
            float(mse_values.std()),
            float(mse_values.min()),
            float(mse_values.max()),
            mse_values.size,
          )

      log_msg = "%s: %.1f (%.3fs)" % (alg[-1], regret.sum(axis=0).mean(), time.time() - start)
      if mse_stats is not None:
        mse_mean, mse_std, mse_min, mse_max, mse_count = mse_stats
        log_msg += ", theta-MSE mean %.4f std %.4f min %.4f max %.4f (n=%d)" % (
          mse_mean, mse_std, mse_min, mse_max, mse_count)
      print("%s, " % log_msg, end="")

      if super_run > 0:
        old_regret = np.load("Results_linear_dmap_new/%s_%s.npy" % (problem, alg[-1].lower()))
        regret = np.hstack((old_regret, regret))
      np.save("Results_linear_dmap_new/%s_%s.npy" % (problem, alg[-1].lower()), regret)
    print()
  ...
  for alg in algs:
    regret = np.load("Results_linear_dmap_new/%s_%s.npy" % (problem, alg[-1].lower()))
  ...
  plt.savefig("Results_linear_dmap_new/%s_%s.pdf" % (problem, alg[-1].lower()))
  plt.close()