...
for problem in problems:
  # If all result npy files already exist for this problem, skip expensive simulation
  result_ready = all(os.path.exists(f"Results_non_linear_cosine_notrain/{problem}_{tag.lower()}.npy")
                     for *_, tag in [a for a in [
                        ("LinTS", "TS"),
                        ("LinDiffTS", "DiffTS"),
                        ("NeuralTS", "NeuralTS"),
                        ("NeuralUCB", "NeuralUCB"),
  ...
  for super_run in range(num_super_runs_local):
    ...
    lin_prior_path = f"Results_non_linear_cosine_notrain/{problem}_diffusion_lin_prior.pkl"
    import joblib
    if os.path.exists(lin_prior_path):
        ...
        lin_prior.no_train(lin_params)
        joblib.dump(lin_prior, lin_prior_path)

     # ---------------- MLP params -> neural diffusion prior ----------------
    diffusion_prior_path = f"Results_non_linear_cosine_notrain/{problem}_neural_prior.pkl"
    if os.path.exists(diffusion_prior_path):
        neural_prior = joblib.load(diffusion_prior_path)
        param_path = f"Results_non_linear_cosine_notrain/{problem}_mlp_params.npy"
      ...
      if os.path.exists(param_path):
        ...
        #           base_sd = {k: v.clone() for k, v in net.state_dict().items()}
          net = SmallMLP()
          params = torch.cat([p.flatten() for p in net.parameters()]).detach().cpu().numpy()
          mlp_param_list.append(params)
      ...
      neural_prior.no_train(mlp_params)
      joblib.dump(neural_prior, diffusion_prior_path)

    out_scatter = f"Results_non_linear_cosine_notrain/{problem}_nonlin_prior_umap.pdf"
    ...
    # plt.tight_layout()
    # plt.legend()
    # plt.savefig("Results_non_linear_cosine_notrain/%s_diffusion_prior.pdf" % problem)
    # plt.close()
    ...
    for run in range(num_runs):
        ...
        envs.append(NonLinearBandit(Phi, theta, sigma=sigma, type='cosine'))
    ...
    algs = [
        ...
        ("NeuralDiffDMAP", {"prior": neural_prior, "sigma": sigma, "K_inner": K_inner, "eta": dmap_eta}, "tab:pink", "-", "DMAPSG")
    ]
    ...
    for alg in algs:
      ...
      if super_run > 0:
        old_regret = np.load("Results_non_linear_cosine_notrain/%s_%s.npy" % (problem, alg[-1].lower()))
        regret = np.hstack((old_regret, regret))
      np.save("Results_non_linear_cosine_notrain/%s_%s.npy" % (problem, alg[-1].lower()), regret)
    print()
  ...
  # ensure algs defined (may be skipped if no simulation)
  if 'algs' not in locals():
      algs = [
          ...
          ("NeuralDiffDMAP", {}, "tab:pink", "-", "DMAPSG"),
      ]
  ...
  # plots
  for alg in algs:
    regret = np.load("Results_non_linear_cosine_notrain/%s_%s.npy" % (problem, alg[-1].lower()))
  ...
  plt.savefig("Results_non_linear_cosine_notrain/%s_%s.pdf" % (problem, alg[-1].lower()))
  plt.close()