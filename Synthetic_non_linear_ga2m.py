...
RESULTS_DIR = "Results_non_linear_ga2m_ga2m"  # <== new output directory
...
sigma = 0.1  # reward noise
n_pretrain = 10000  # number of samples for estimating the prior

# Default exploration step size for DPSG (can be overridden per problem)
eta = 0.001
...
for problem in problems:
  ...
  for super_run in range(num_super_runs_local):
    ...
    plt.tight_layout()
    plt.legend()      
    plt.savefig("Results_non_linear_ga2m_ga2m/%s_diffusion_prior.pdf" % problem)
    plt.close()
    ...
    for run in range(num_runs):
        ...
        envs.append(NonLinearBandit(Phi, theta, sigma=sigma, type='ga2m'))
