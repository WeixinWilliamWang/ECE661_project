...
# --- helper to filter chosen algorithms ---

def filter_algorithms(algs, selection):
    if selection.lower() == "all":
        return algs
    req = {tok.strip().lower() for tok in selection.split(',') if tok.strip()}
    return [a for a in algs if a[4].lower() in req or a[0].lower() in req]
