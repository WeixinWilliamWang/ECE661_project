...
for problem in problems:
  ...
  for super_run in range(num_super_runs):
    ...
    lin_prior_path = f"Results_two_layer_2000_two_layer/{problem}_diffusion_lin_prior.pkl"
    import joblib
    ...
    diffusion_prior_path = f"Results_two_layer_2000_two_layer/{problem}_neural_prior.pkl"
    if os.path.exists(diffusion_prior_path):
        neural_prior = joblib.load(diffusion_prior_path)
        param_path = f"Results_two_layer_2000_two_layer/{problem}_mlp_params.npy"
    ...
    out_scatter = f"Results_two_layer_2000_two_layer/{problem}_nonlin_prior_umap.pdf"
    ...
    plt.tight_layout()
    plt.legend()
    plt.savefig("Results_two_layer_2000_two_layer/%s_diffusion_prior.pdf" % problem)
    plt.close()
    ...
    for alg in algs:
      ...
      res_path = "Results_two_layer_2000_two_layer/%s_%s.npy" % (problem, alg[-1].lower())
      loaded = False
      if os.path.exists(res_path):
          regret = np.load(res_path)
          if regret.shape[0] == n:
              cum_per_run = regret.sum(axis=0)
              group_size = 16
              if cum_per_run.size >= group_size:
                  num_groups = cum_per_run.size // group_size
                  group_means = [cum_per_run[i*group_size:(i+1)*group_size].mean() for i in range(num_groups)]
                  group_stds = [cum_per_run[i*group_size:(i+1)*group_size].std() for i in range(num_groups)]
                  print("%s: %.1f (Loaded) Means: %s Stds: %s, " % (alg[-1], regret.sum(axis=0).mean(), group_means, group_stds), end="")
              else:
                  print("%s: %.1f (Loaded) [Runs < 16, no groups], " % (alg[-1], regret.sum(axis=0).mean()), end="")
              loaded = True
          else:
              print(f" [Shape mismatch: expected {n}, got {regret.shape[0]}. Re-running...] ", end="")
      ...
      if not loaded:
          ...
          if cum_per_run.size >= group_size:
              ...
              group_means = [cum_per_run[i*group_size:(i+1)*group_size].mean() for i in range(num_groups)]
              group_stds = [cum_per_run[i*group_size:(i+1)*group_size].std() for i in range(num_groups)]
              print("%s: %.1f (%.3fs) Means: %s Stds: %s, " % (alg[-1], regret.sum(axis=0).mean(), time.time() - start, group_means, group_stds), end="")
          else:
              print("%s: %.1f (%.3fs), " % (alg[-1], regret.sum(axis=0).mean(), time.time() - start), end="")

          if super_run > 0:
            old_regret = np.load("Results_two_layer_2000_two_layer/%s_%s.npy" % (problem, alg[-1].lower()))
            regret = np.hstack((old_regret, regret))
          np.save("Results_two_layer_2000_two_layer/%s_%s.npy" % (problem, alg[-1].lower()), regret)
    print()
  ...
  # plots
  for alg in algs:
    regret = np.load("Results_two_layer_2000_two_layer/%s_%s.npy" % (problem, alg[-1].lower()))
    # Print cumulative regret grouped into 4 super runs of 16 runs each
    if regret.shape[1] == 64:  # ensure we have the expected number of runs
        cum_per_run = regret.sum(axis=0)  # cumulative regret per run
        group_size = 16
        num_groups = cum_per_run.size // group_size
        group_means = [cum_per_run[i*group_size:(i+1)*group_size].mean() for i in range(num_groups)]
        group_stds = [cum_per_run[i*group_size:(i+1)*group_size].std() for i in range(num_groups)]
        print(f"Problem {problem}, Algorithm {alg[-1]} grouped cumulative regret (4x16): Means: {group_means}, Stds: {group_stds}")
  ...
  plt.savefig("Results_two_layer_2000_two_layer/%s_%s.pdf" % (problem, alg[-1].lower()))
  plt.close()