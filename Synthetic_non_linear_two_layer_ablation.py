...
for problem in problems:
  ...
  for super_run in range(num_super_runs_local):
    ...
    for alg in algs:
      ...
      agg_inference.setdefault(key, []).append(inference_time)
      curr_inference.setdefault(key, []).append(inference_time)

      print(f"Problem {problem} | Alg {alg[4]}{extra} | Final cumulative regret: mean={final_mean:.4f} ± {final_std:.4f} (SD) | inference_time {inference_time:.3f}s  -> saved {out_path}")

  # ---- after all super runs, print aggregate statistics ----
  print(f"\n[Summary over {num_super_runs_local} super runs | Problem {problem}]")
  for name in agg_final.keys():
      vals_reg = np.asarray(agg_final[name])
      vals_inf = np.asarray(agg_inference.get(name, []))
      print(f"  {name}: regret mean={vals_reg.mean():.4f} ± {vals_reg.std():.4f} (SD), inference time mean={vals_inf.mean():.3f}s ± {vals_inf.std():.3f}s over {len(vals_reg)} runs")

    # --- After finishing algorithms for this super run, optionally print inference summary ---
    if super_run == 0:
      print(f"\n[Inference time after first super run | Problem {problem}]")
      for name, vals in curr_inference.items():
          vals = np.asarray(vals)
          print(f"  {name}: inference time mean={vals.mean():.3f}s ± {vals.std():.3f}s over {len(vals)} runs")
